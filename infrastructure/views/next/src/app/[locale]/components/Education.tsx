  ...
  return (
    <div className="bg-muted-foreground">
      <SectionContainer id={navItem?.id} aria-label={navItem?.title}>
        <h3 className="border-b-4 border-primary text-4xl font-bold text-paragraph-foreground sm:text-5xl">
          {navItem?.title}
        </h3>
        <main className="w-full max-w-4xl">
          <Card className="w-full items-start gap-0 border-2 border-primary bg-transparent p-1 text-background sm:p-3">
            <h3 className="font-mono text-2xl font-bold text-paragraph-foreground">
              {t("university.title")}
            </h3>
          ...
          <h3 className="mt-10 w-fit border-b-2 border-primary text-2xl font-semibold text-paragraph-foreground">
            {t("courses.title")}
          </h3>
          <div className="mt-4 grid gap-4">
            {courses.map((course) => (
              <Card
                key={course.title}
                className="w-full items-start gap-0 border border-primary bg-transparent p-1 text-background sm:p-3"
              >
                <h3 className="font-mono text-lg font-bold text-paragraph-foreground">
                  {course.title}
                </h3>
